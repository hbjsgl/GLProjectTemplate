...
@implementation UIDevice (GLHardware)
...
+ (NSString *)gl_platformString{
    NSString *platform = [self gl_platform];
...
+ (NSUInteger)gl_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)gl_cpuFrequency {
    return [self gl_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)gl_busFrequency {
    return [self gl_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)gl_ramSize {
    return [self gl_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)gl_cpuNumber {
    return [self gl_getSysInfo:HW_NCPU];
}


+ (NSUInteger)gl_totalMemoryBytes
{
    return [self gl_getSysInfo:HW_PHYSMEM];
}
